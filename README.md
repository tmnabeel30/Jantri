...
## 🧑‍💻 Smoke Test (optional)
Add this once (e.g., `onAppear`) to verify Firestore writes:

```swift
import FirebaseFirestore

Firestore.firestore().collection("debug").document("ping")
  .setData(["ts": Timestamp(date: Date())]) { err in
    print(err == nil ? "Firestore write OK" : "Firestore write FAILED: \(err!)")
  }
```
...
### JSON → Core Data
- Create `ColorModel.xcdatamodeld` with `ColorEntity` (`id:String`, `hex:String`, `createdAt:Date`, `isSynced:Bool`).
- Implement `CoreDataLocalStore: LocalStore` (a sample is in the project notes/canvas).
- Switch in DI: `let store: LocalStore = CoreDataLocalStore()`.

### Firestore → Mock (tests/previews)
- Implement `MockCloudSyncService: CloudSyncing` that returns the IDs passed in.
- Inject via DI for previews/tests (already done under `PreviewSupport`).

---

## ✅ Unit Tests (example)
Use in‑memory store + mock cloud for fast, deterministic tests.

```swift
import XCTest
@testable import ColorSync

final class ColorSyncTests: XCTestCase {
  func testSyncMarksItemsSynced() async throws {
    let store = InMemoryStore() // LocalStore stub
    try store.save([ColorItem(id: "1", hex: "#FF00AA", createdAt: Date(), isSynced: false)])

    var cloud = MockCloudSyncService()
    let repo = ColorRepository(store: store, cloud: cloud)

    _ = try await repo.syncIfNeeded(online: true)
    let all = await repo.all()
    XCTAssertTrue(all.first?.isSynced == true)
  }
}
```
...
### Credits
Built with ❤️ using SwiftUI, MVVM, and Firebase.
